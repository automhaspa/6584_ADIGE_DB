...
SET ANSI_NULLS ON
GO
CREATE PROC [dbo].[sp_Stocca_Udc_Da_Compattare]
	@Id_Udc						INT,
	@CompattazioneConclusa		BIT = 0,
	-- Parametri Standard;
...
BEGIN
	...
	BEGIN TRY
		...
		IF EXISTS (SELECT TOP 1 1 FROM dbo.Udc_Posizione WHERE ID_UDC = @Id_Udc AND Id_Partizione = @Id_Partizione_Destinazione)
			THROW 50009, 'UDC VUOTA E GIA'' PRESENTE SULLA BAIA DI SCARICO. RIMUOVERE L''UDC',1
		...
		IF @CompattazioneConclusa = 1
			UPDATE	dbo.Udc_Testata
			SET		Da_Compattare = 0
			WHERE	Id_Udc = @Id_Udc
