...
CREATE PROCEDURE [dbo].[sp_Insert_Crea_Missioni_Modula]
	@Id_Udc			INT,
	@Id_Testata		INT,
	--LOAD LINE ID
	@NUMERO_RIGA	INT,
...
BEGIN
	...
	BEGIN TRY
		...
		IF EXISTS	(
			...
			THROW 50001, 'CONTROLLARE TABELLA DI SCAMBIO MODULA HOST_INCOMING_ORDERS RECORD DUPLICATO DI TESTATA',1
		...
		SELECT	toe.LOAD_ORDER_ID, roe.ITEM_CODE, CONCAT(ISNULL(roe.PURCHASE_ORDER_ID, 'n'), '_' , roe.LOAD_LINE_ID), ud.Quantita_Pezzi,
				TOE.LOAD_ORDER_TYPE, cast(roe.FL_INDEX_ALIGN as varchar(40)), roe.MANUFACTURER_ITEM, roe.MANUFACTURER_NAME, NULL
		...
		SELECT	toe.LOAD_ORDER_ID, TOE.LOAD_ORDER_TYPE, ISNULL(toe.SUPPLIER_CODE, ' '), ISNULL(toe.DES_SUPPLIER_CODE, ' '), ISNULL(toe.SUPPLIER_DDT_CODE, ' '),
				ISNULL(CAST(toe.DT_RECEIVE_BLM AS date), ' '), NULL, ' ', ' '
		FROM	Udc_Dettaglio					UD
		JOIN	Custom.TestataOrdiniEntrata		toe
		ON		ud.Id_Ddt_Reale = toe.ID
		WHERE	Id_Ddt_Reale = @Id_Testata
