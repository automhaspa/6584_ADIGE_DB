...
BEGIN
	...
	BEGIN TRY
		...
		DECLARE	@BEHMG				NUMERIC(10, 3)
		DECLARE	@PKBHT				VARCHAR(15)
		...
		DECLARE CursoreMovimenti CURSOR LOCAL FAST_FORWARD FOR
			SELECT	HL.PRG_MSG,
					...
					ISNULL(HL.BEHMG,RLP.BEHMG),
					ISNULL(HL.PKBHT,RLP.PKBHT),
					ISNULL(HL.ABLAD,RLP.ABLAD)
		...
		BEGIN
			BEGIN TRY
				...
				BEGIN
					...
					INSERT INTO Custom.AnagraficaMancanti
						(Id_Testata,Id_Riga,Id_Articolo,Qta_Mancante,WBS_Riferimento, COMM_PROD, COMM_SALE,PROD_ORDER,SAP_DOC_NUM, PROD_LINE,
							ORDER_ID,ORDER_TYPE,DT_EVASIONE, RagSoc_Dest,BEHMG,PKBHT,ABLAD)
					VALUES
						(@ID, @LINE_ID, @IdArticoloAwm, @Quantity,@WBS_ELEM,@COMM_PROD,@COMM_SALE,@PROD_ORDER,@SAP_DOC_NUMBER,@PROD_LINE,
							@ORDER_ID,@ORDER_TYPE,@DT_EVASIONE,@DES_PREL_CONF,@BEHMG,@PKBHT,@ABLAD)
				END
