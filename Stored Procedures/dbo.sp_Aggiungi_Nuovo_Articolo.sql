...
BEGIN
	...
	BEGIN TRY
		...
		--IF NOT EXISTS (SELECT TOP 1 1 FROM Partizioni WHERE ID_TIPO_PARTIZIONE IN ('SP','AT','PK','KT','US') AND ID_PARTIZIONE = @Id_Partizione_Udc)
		--	AND NOT EXISTS (SELECT TOP 1 1 FROM AwmConfig.vDestinazioni WHERE ID_PARTIZIONE = @Id_Partizione_Udc)
		--	THROW 50001, 'OPERAZIONE AUTORIZZATA SOLO PER LE UDC IN BAIA DESTINAZIONE O AREA TERRA',1
		...
		IF 
			@Id_Partizione_Udc IN (3404, 3604, 3203)
				AND
			EXISTS	(
						SELECT	TOP 1 1
